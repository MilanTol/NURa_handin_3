...
To find the parameters $a,b,c$ that minimize $\chi^2$, 
we employ the Levenberg-Marquardt algorithm, which is designed specifically for $\chi^2$ optimization.
We employ this algorithm despite the fact that the data is generated poissonically: the data 
consists of discrete counts in bins. 

We pass the counts in each bin and the choice of bins into the algorithm.
...
We perform this integration with the romberg algorithm which uses an open method to handle the undefined behaviour at x=0. 
...
in saddles. We also set the initial guess of the parameters to 
$a=1.5, b=0.8, c=2.5$, as these produce reasonable results. 
The algorithm seems to not be very stable, as varying the initial parameters to 
worse guesses sometimes leads to diverging runs. The method 
becomes more like steepest descent after unsuccesful proposed parametrizations, 
which is a very stable algorithm to find minima, assuming the parameter space does not 
contain local minima. So this might be an indication that the parameter space
is quite challging and does contain such local minima.
