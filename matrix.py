...
class Matrix:
    ...
    def inverse(self):
        ...
        for i in range(N):  # loop over columns,
            ...
            for row in range(
                N
            ):  # subtract pivot row from all other rows with non-zero elements in column i
                if row == i:
                    continue
                if mat[row, i] == 0:
                    continue
    ...
    def LU_decomposition(self):
        ...
        self.LU_indx = Matrix(
            range(N), dtype=int
        )  # placeholder vector to store indices of rows with largest pivot candidates
    ...
    def solve(self, b):
        ...
        if self.shape[1] != b.shape[0]:
            raise Exception(
                "shape of b (number of rows) does not match matrix (number of columns)"
            )
        ...
        x = x[
            self.LU_indx.data
        ]  # swap rows of b similar to how we swapped rows when choosing pivots for LU decomposition
        ...
        for i in range(
            N - 1, -1, -1
        ):  # start at N-1, go up to and including 0, with steps -1
