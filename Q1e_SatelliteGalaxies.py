...
def do_question_1e():
    ...
    chi2_sampler = Distribution(
        N_chi2, xmin=x_lower, xmax=x_upper, seed=1
    )  
    poisson_sampler = Distribution(
        N_poisson, xmin=x_lower, xmax=x_upper, seed=2
    )
    ...
    for i in range(num_pseudo_experiments):
        ...
        N_samples=Nsat, pmax=1.01*N_max_chi2
        )
        pseudo_radius_poisson = poisson_sampler.rejection(
            N_samples=Nsat, pmax=1.01*N_max_poisson
        )
