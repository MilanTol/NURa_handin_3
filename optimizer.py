...
class Minimizer:
    ...
    def bracket(self, a:float, b:float, maxit:int=100) -> tuple[float, float, float]:
        ...
        for i in range(maxit):
            ...
            d = parabola_minimum(a, b, c, f_a, f_b, f_c) # fit parabola to abc, store minimum 
            ...
            if b < d and d < c: # check whether d is in between b and c.
                ...
                if np.abs(d-b)>100*np.abs(c-b): # check whether its too far beyond.
    ...
    def tighten(self, bracket:tuple[float,float,float], abserr:float=1e-5, relerr:float=1e-5, maxit:int=100) -> float:
        ...
        for i in range(maxit): 
            ...
            if np.abs(c-a) < abserr or np.abs(c-a)*b < relerr: # check whether target accuracy is reached
